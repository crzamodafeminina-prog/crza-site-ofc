...
const CORES = {
  terracota: { nome: "Terracota", hex: "#AF4E2F" },
  areia:     { nome: "Areia",     hex: "#D6C4AB" },
  offwhite:  { nome: "Off-white", hex: "#EFE9DF" },
  cacau:     { nome: "Cacau",     hex: "#3B2A21" },
  oliva:     { nome: "Oliva",     hex: "#7A7A5E" },
};

const PRODUTOS = [
  { id:"aura",   nome:"Aura",   preco:110, categoria:"Básico",     modelagem:"Cortininha",       cores:["terracota","areia","cacau"],    tecido:"Poliamida com proteção UV", bojo:true,  destaque:true,  descricao:"Cortininha de amarração regulável nos dois lados. Bojo removível e lastex nas costas — segura firme sem marcar." },
  { id:"bali",   nome:"Bali",   preco:100, categoria:"Básico",     modelagem:"Tomara que caia",  cores:["areia","offwhite","oliva"],     tecido:"Poliamida com proteção UV", bojo:true,  destaque:true,  descricao:"Tomara que caia com lastex duplo e bojo firme. Calcinha de cintura média, cobertura moderada atrás." },
  { id:"duna",   nome:"Duna",   preco:90,  categoria:"Básico",     modelagem:"Meia taça",        cores:["cacau","terracota","areia"],    tecido:"Poliamida com proteção UV", bojo:false, destaque:true,  descricao:"Meia taça de alcinha fina, sem bojo. A peça mais leve da linha, pensada para dias de praia longos." },

  /* Exemplos para preencher a vitrine — edite ou apague estes três */
  { id:"ipe",    nome:"Ipê",    preco:120, categoria:"Coleção",    modelagem:"Asa delta",        cores:["terracota","cacau"],            tecido:"Poliamida canelada",        bojo:true,  destaque:false, descricao:"Asa delta de cava alta em canelado. Calcinha fio com laterais reguláveis." },
  { id:"salina", nome:"Salina", preco:105, categoria:"Coleção",    modelagem:"Cortininha",       cores:["offwhite","areia","oliva"],     tecido:"Poliamida canelada",        bojo:true,  destaque:false, descricao:"Cortininha canelada com acabamento em viés. Calcinha de cintura alta." },
  { id:"cacau",  nome:"Cacau",  preco:95,  categoria:"Coleção",    modelagem:"Meia taça",        cores:["cacau","oliva"],                tecido:"Poliamida fosca",           bojo:false, destaque:false, descricao:"Meia taça franzida no centro, sem bojo, com alças cruzadas nas costas." },
];
...
/* =========================================================================
   ...
   ========================================================================= */
const PALETAS = [
  ["#C96A43", "#8E3C22", "#E7DAC7"],
  ["#D6C4AB", "#AF4E2F", "#F3EDE3"],
  ["#7A7A5E", "#3B2A21", "#E2D4BE"],
  ["#B98A5E", "#6E3A22", "#EFE5D6"],
];
...
function Cabecalho({ pagina, irPara, contagem, abrirCarrinho, abrirBusca }) {
  ...
  const links = [
    ["inicio", "Início"], ["biquinis", "Biquínis"], ["colecao", "Coleção"],
      ...
      <div className="relative w-full max-w-[1280px] mx-auto px-5 md:px-10 pb-16 md:pb-24">
        <div className="max-w-[640px]">
          <Titulo nivel="h1" className="text-offwhite text-[46px] sm:text-[64px] md:text-[86px] subir" style={{ animationDelay: "200ms" }}>
            O sol já sabe<br />o seu nome.
          </Titulo>
          <p className="text-offwhite/80 text-[16px] md:text-[18px] leading-relaxed mt-6 max-w-[430px] subir" style={{ animationDelay: "340ms" }}>
            Biquínis feitos em tons de terra para a pele que eles foram pensados para valorizar.
            Corte limpo, caimento firme, nada sobrando.
          </p>
      ...
      <div className="flex flex-wrap items-end justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-cacau/60 mt-3 max-w-[420px] leading-relaxed">
            Os três cortes da linha Básico, feitos para repetir o verão inteiro.
          </p>
        </div>
...
function Identidade() {
  const pontos = [
    ...
    ["Cores escolhidas na pele", "Terracota, areia e cacau foram testados em pele negra antes de virar coleção — não depois."],
  ];
  return (
      ...
      <div className="max-w-[1280px] mx-auto px-5 md:px-10 py-20 md:py-28 grid lg:grid-cols-[0.95fr_1.05fr] gap-12 lg:gap-20 items-center">
        <div className="relative">
          ...
          <div className="absolute -bottom-6 -right-4 md:-right-8 bg-cacau text-offwhite px-7 py-6 max-w-[210px]">
            ...
            <p className="text-offwhite/65 text-[13px] mt-2 leading-snug">De {CONFIG.cidade} para o litoral inteiro.</p>
          </div>
        </div>
        <div>
          <Titulo className="text-[36px] md:text-[54px] max-w-[500px]">
            Biquíni não é tendência. É o jeito de estar no corpo.
          </Titulo>
          <p className="text-cacau/70 leading-relaxed mt-6 max-w-[520px]">
            A CRZA nasceu de uma inconformidade simples: as fotos nunca mostravam a nossa pele, e as cores
            nunca pareciam pensadas para ela. A gente começou pela paleta e desenhou tudo a partir dali —
            os cortes, o tecido, a forma de vestir.
          </p>
    ...
    <section className="relative overflow-hidden bg-cacau">
      ...
      <div className="relative max-w-[1280px] mx-auto px-5 md:px-10 py-24 md:py-36">
        <div className="max-w-[520px]">
          <p className="text-terra text-[13px] tracking-[0.22em] mb-5">Chega em outubro</p>
          <Titulo className="text-offwhite text-[40px] md:text-[64px]">Coleção Maresia</Titulo>
          <p className="text-offwhite/75 leading-relaxed mt-5 max-w-[440px]">
            Canelados, cavas altas e dois tons novos de terra. Quem entrar na lista compra dois dias antes.
          </p>
          <div className="mt-9">
...
function PaginaColecao({ onAbrir, irPara }) {
  ...
  const daColecao = PRODUTOS.filter((p) => p.categoria === "Coleção");
  ...
  return (
    <div className="pt-[68px]">
      <section className="relative bg-cacau overflow-hidden">
        ...
        <div className="relative max-w-[1280px] mx-auto px-5 md:px-10 py-24 md:py-36">
          <Titulo nivel="h1" className="text-offwhite text-[44px] md:text-[76px]">Maresia</Titulo>
          <p className="text-offwhite/75 leading-relaxed mt-5 max-w-[460px]">
            Seis peças em canelado, cavas altas e dois tons inéditos. Pré-venda em outubro, lote único.
          </p>
        </div>
      </section>
      ...
      <section className="bg-areia">
        <div className="max-w-[720px] mx-auto px-5 md:px-10 py-20 md:py-24 text-center">
          ...
          {enviado ? (
            <p className="mt-8 text-terra">Pronto — você entrou na lista. Avisamos por e-mail assim que abrir.</p>
          ) : (
          ...
          <p className="text-[12px] text-cacau/45 mt-5">
            Prefere no WhatsApp? <a href={zap("Quero entrar na lista da coleção Maresia.")} target="_blank" rel="noopener noreferrer" className="text-terra border-b border-terra/40">chama aqui</a>.
          </p>
        </div>
      </section>
    </div>
  ...
  return (
    <div className="pt-[68px]">
      <section className="max-w-[1280px] mx-auto px-5 md:px-10 pt-16 md:pt-24 pb-16 grid lg:grid-cols-[1.05fr_0.95fr] gap-12 lg:gap-20 items-start">
        <div>
          <Titulo nivel="h1" className="text-[42px] md:text-[68px] max-w-[520px]">Começamos pela cor.</Titulo>
          <div className="mt-8 space-y-5 text-cacau/70 leading-relaxed max-w-[540px]">
            <p>
              A CRZA é uma marca pequena de biquínis feita em {CONFIG.cidade}. Começou com uma vitrine de
              Instagram e três modelagens — as mesmas três que ainda são a nossa linha Básico.
            </p>
            <p>
              A paleta veio antes de tudo. Terracota, areia, cacau e oliva foram escolhidos testando tecido
              contra pele negra na luz do sol, não numa tela de computador. Só depois desenhamos os cortes.
            </p>
            <p>
              Produzimos em lote pequeno, com costureiras que conhecemos pelo nome. Se uma peça esgota,
              ela volta quando dá — não antecipamos estoque para inflar vitrine.
            </p>
          </div>
      ...
      <aside className="absolute right-0 top-0 bottom-0 w-full sm:w-[440px] bg-offwhite flex flex-col shadow-2xl"
        ...
        {itens.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center px-10">
            <p className="text-cacau/55 leading-relaxed">Sua sacola está vazia. Comece pela linha Básico — três cortes, três tons.</p>
        ...
        <div className="grid gap-12 md:grid-cols-[1.4fr_1fr_1fr_1.2fr]">
          <div>
            ...
            <p className="text-offwhite/60 text-[14px] leading-relaxed mt-4 max-w-[260px]">
              Biquínis em tons de terra, feitos em lote pequeno em {CONFIG.cidade}.
            </p>
          ...
          <nav aria-label="Loja">
            ...
            <ul className="space-y-2.5 text-[15px]">
              {[["biquinis", "Biquínis"], ["colecao", "Coleção Maresia"], ["sobre", "Tabela de medidas"]].map(([id, r]) => (
